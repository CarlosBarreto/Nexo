...
| Claude CLI spawn | `houston-terminal-manager/src/claude_runner.rs` (chat) + `claude_command.rs::claude_command_name` + `houston-engine-core/src/sessions/provider_oneshot.rs::run_claude` (one-shot summarize / generate) | runtime-installed absolute path `~/.local/bin/claude` when present (`claude_install_path::is_installed`), else bare `claude` on the resolved PATH | runtime-installed absolute path `%LOCALAPPDATA%\Programs\claude\claude.exe` when present, else bare `claude`. **Both chat and one-shot resolve via the same `claude_command_name()`** — HOU-445 was the one-shot path spawning bare `Command::new("claude")` while the runtime installer drops `claude.exe` under `%LOCALAPPDATA%`, a dir that only joins the start-up-cached `shell_path()` if it already existed at init; installed-after-startup left the cached PATH without it, so generate/summarize died with `spawn failed: program not found` while chat (absolute path) kept working. |
| Codex CLI spawn | `houston-terminal-manager/src/codex_runner.rs` (chat) + `houston-engine-core/src/sessions/provider_oneshot.rs::run_codex` (one-shot) | bundled codex (`bundled_codex_path`, universal binary in `Resources/bin/`), else bare `codex` on PATH | bundled `codex.exe` shipped for `windows-x64`, else bare `codex` — chat and one-shot use the identical resolver so neither can drift from the other |
