 ...
 * surfaced by `@nexo-ai/engine-client` as a `NexoEngineError` whose
 * `.status` is 404.
 *
 * Unlike the legacy Rust engine, the TS host emits bare-string error bodies, so
 * there is NO typed `.kind` on this error (`NexoEngineError.kind` reads
 ...
 * structural `.status` exposed by `NexoEngineError` (and tolerates a plain
...
export function isMissingSkillError(err: unknown): boolean {
  ...
  // Primary signal: the engine-client NexoEngineError for a 404 skill GET.
