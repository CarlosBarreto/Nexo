...
// Dev-only sign-in fallback: the `nexo://` deep link opens the installed
...
describe("toCallbackUrl (dev sign-in paste)", () => {
  ...
  it("passes a full https callback URL through unchanged", () => {
    const u = "https://getnexo.ai/auth/callback/?code=abc-123";
    strictEqual(toCallbackUrl(u), u);
  });

  it("passes a nexo:// deep link through unchanged", () => {
    const u = "nexo://auth-callback?code=abc-123";
    strictEqual(toCallbackUrl(u), u);
  });

  it("wraps a bare code into a nexo:// callback URL", () => {
    strictEqual(
      toCallbackUrl("c686974e-6f9f-4742-aff5-1d30761eeb7a"),
      "nexo://auth-callback?code=c686974e-6f9f-4742-aff5-1d30761eeb7a",
    );
  });
  ...
  it("extracts the code from a copied query fragment", () => {
    strictEqual(
      ...
      "nexo://auth-callback?code=abc-123",
    );
  });
